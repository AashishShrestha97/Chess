...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!authLoading && user) {
      ...
      console.warn("⚠️ User not authenticated in ProfilePage");
      setLoading(false);
    }
  }, [authLoading, user]);
  ...
  const fetchProfileData = async () => {
    try {
      ...
      console.log("📊 Performance response:", performanceRes.status);
      console.log("📊 Games response:", gamesRes.status);
      ...
      if (!gamesRes.ok) {
        console.error("❌ Games fetch failed:", await gamesRes.text());
      }

      if (statsRes.ok && performanceRes.ok && gamesRes.ok) {
        ...
        console.error("❌ One or more profile endpoints returned error");
      }
    } catch (error) {
  ...
  if (!user) {
    return (
      <div className="profile-container">
        <div className="error">❌ Not authenticated. Please log in first.</div>
      </div>
    );
  }
                ...
                <div className="results-list">
                  <div className="result-item">
                    <span className="result-label">
                      Wins ({Math.round((stats.wins / stats.gamesPlayed) * 100)}
                      %)
                    </span>
